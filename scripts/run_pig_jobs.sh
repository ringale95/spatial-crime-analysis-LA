...
# pig -x mapreduce -f pig-scripts/load/load_crime_data.pig > $LOG_DIR/load_$(date +%Y%m%d_%H%M%S).log 2>&1
# if [ $? -ne 0 ]; then
...
# pig -x mapreduce -f pig-scripts/utils/cleanup.pig > $LOG_DIR/cleanup_$(date +%Y%m%d_%H%M%S).log 2>&1
# if [ $? -ne 0 ]; then
